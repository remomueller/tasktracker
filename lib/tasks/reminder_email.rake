...
task reminder_email: :environment do
  ...
  User.current.where(emails_enabled: true).each do |user|
    if user.all_deliverable_stickies_due_today.size + user.all_deliverable_stickies_past_due.size + user.all_deliverable_stickies_due_upcoming.size + user.digest_stickies_created.size + user.digest_stickies_completed.size + user.digest_comments.size > 0
