...
class TemplatesControllerTest < ActionController::TestCase
  ...
  test 'should update template' do
    patch :update, id: @template.to_param, template: @template.attributes
    assert_redirected_to template_path(assigns(:template))
  end

  test 'should not update template with blank name' do
    patch :update, id: @template, template: { name: '', project_id: projects(:one).to_param, item_tokens: [ { description: 'Reminder in a Week', interval: 1, units: 'weeks', owner_id: users(:valid).to_param } ] }
    assert_not_nil assigns(:template)
    assert assigns(:template).errors.size > 0
    assert_equal ["can't be blank"], assigns(:template).errors[:name]
  ...
  test 'should not update template with blank project' do
    patch :update, id: @template, template: { name: 'Updated Name', project_id: nil, item_tokens: [ { description: 'Reminder in a Week', interval: 1, units: 'weeks', owner_id: users(:valid).to_param } ] }
    assert_not_nil assigns(:template)
    assert assigns(:template).errors.size > 0
    assert_equal ["can't be blank"], assigns(:template).errors[:project_id]
    assert_template 'edit'
  ...
  test 'should not update template with invalid id' do
    patch :update, id: -1, template: { name: 'Updated Name', project_id: nil, item_tokens: [ { description: 'Reminder in a Week', interval: 1, units: 'weeks', owner_id: users(:valid).to_param } ] }
    assert_nil assigns(:template)
    assert_redirected_to templates_path
  end
