...
class GroupsControllerTest < ActionController::TestCase
  ...
  test "should create group of stickies avoid weekends" do
    ...
    assert_equal boards(:one), assigns(:board)
    assert_equal users(:valid), assigns(:group).user
    assert_equal Time.local(2012, 3, 9, 0, 0, 0).to_time, assigns(:group).stickies.order('due_date').first.due_date.to_time
    assert_equal Time.local(2012, 3, 12, 0, 0, 0).to_time, assigns(:group).stickies.order('due_date').last.due_date.to_time
    assert_redirected_to group_path(assigns(:group))
  end
