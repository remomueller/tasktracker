...
* Bug Fix
  ...
  * When deleting frames, tags, and templates, the user is now redirected to the project specific frames, tags, or templates index
  * Frames now correctly retain the dropdown arrow when switching by selecting a new frame from the dropdown
...
* Enhancements
  ...
  * Template Changes:
    * Frames and Templates now require unique names per project
...
* Enhancements
  * Creating a sticky from the project page will
    * default the new sticky to the currently selected frame
    * refresh the list of stickies when the sticky is created
  * Switching frames in the sticky popup will now reflect on the underlying project page
  * Update to Rails 3.2.6 and Contour 1.0.2
  * Links with confirm: now use data: { confirm: } to account for deprecations in Rails 4.0
  * Frames are now ordered by end date descending by default on frames index
...
* Bug Fix
  * Generating a template from the calendar view now correctly updates the selected templates available project frames
...
* Refactoring
  ...
  * Notes RSS feed has moved to Task Tracker
...
* Enhancements
  ...
  * Navigation
    ...
    * Viewing a project now adds a menu item to that project's templates and frames
  * Sticky Tags:
...
* Bug Fix
  ...
  * Validation errors when creating or updating stickies now correctly remember the project, frame, and tag selections
