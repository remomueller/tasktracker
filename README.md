...
```
SHELL=/bin/bash
0 1 * * * source /etc/profile.d/rvm.sh && cd /var/www/tasktracker && /usr/local/rvm/gems/ruby-2.2.3/bin/bundle exec rake reminder_email RAILS_ENV=production
```
...
```
SHELL=/bin/bash
30 1 * * * source /etc/profile.d/rvm.sh && cd /var/www/tasktracker && /usr/local/rvm/gems/ruby-2.2.3/bin/bundle exec rake export_dictionary RAILS_ENV=production
```
